"""
Inline web UI assets for the Plasmid Designer (HTML_PAGE).

This module holds the full HTML/CSS/JS for the single-page chat interface.
Kept separate from app.py so the HTTP handler stays readable — the page is
thousands of lines of markup that rarely needs editing.

VIEWER_CSS and VIEWER_JS from viewer_assets.py are spliced in at module
load time (replacing </style> and </body> anchors) so the page is built
once at startup rather than per-request.

app/app.py imports HTML_PAGE and serves it for GET / requests.
"""

# app/ may or may not be on sys.path depending on how the server is launched.
# Match the same try/except fallback pattern used elsewhere in the app package.
try:
...
# Splice the Ori-style inline sequence viewer into the page at startup (once).
# CSS goes just before </style>; JS goes just before </body>. Both anchors
# are unique in HTML_PAGE so count=1 is a safety guard, not a necessity.
