...
def search_backbones(query: str, organism: str = None, promoter: str = None) -> list[dict]:
...
def search_inserts(query: str, category: str = None) -> list[dict]:
...
def get_backbone_by_id(backbone_id: str) -> dict | None:
...
def get_insert_by_id(insert_id: str) -> dict | None:
...
def validate_dna_sequence(sequence: str) -> dict:
    ...
    if result["is_valid"]:
        ...
        result["gc_content"] = round(gc_count / len(clean_seq) * 100, 1) if len(clean_seq) > 0 else 0
