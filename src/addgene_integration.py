...
# ────────────────────────────────────────────────────────────────────────────
# GenBank parsing — module-level functions, reusable outside AddgeneClient
# (e.g., by src/user_library.py for bring-your-own-library support)
# ────────────────────────────────────────────────────────────────────────────
...
class AddgeneClient:
    ...
    # ── GenBank parsing: thin wrappers delegating to module-level functions ──
    # (Kept for backward compat; new code should use the module-level parse_genbank* functions)

    def _extract_sequence_from_genbank(self, content: str) -> Optional[str]:
        return parse_genbank_sequence(content)

    @staticmethod
    def _parse_genbank_location(location_str: str) -> tuple[int, int]:
        return parse_genbank_location(location_str)

    def _parse_genbank_features(self, content: str) -> tuple[List[Dict], Optional[Dict]]:
        return parse_genbank_features(content)
