...
from src.genbank_utils import _PLANNOTATE_AVAILABLE
...
from src.genbank_utils import (  # noqa: E402
...
class TestFormatAsGenbank:
    ...
    def _genbank(self, sequence=SIMPLE_SEQ, name="test", insert_name="GFP",
                 ...
                 reverse_complement_insert=False):
        with patch("src.genbank_utils.annotate", return_value=_empty_df()):
...
class TestGetPlasmidPlotJson:
    ...
    def _plot_json(self, df=None):
        if df is None:
            df = _empty_df()
        mock_plot = MagicMock()
        with patch("src.genbank_utils.get_bokeh", return_value=mock_plot), \
             patch("src.genbank_utils.json_item", return_value={"doc": {"roots": []}, "version": "2.4.1"}):
    ...
    def test_sizing_mode_set_on_plot(self):
        ...
        mock_plot = MagicMock()
        with patch("src.genbank_utils.get_bokeh", return_value=mock_plot), \
             patch("src.genbank_utils.json_item", return_value={}):
...
class TestExportGenbankWithPlot:
    ...
    def _export(self, sequence=SIMPLE_SEQ, name="test", insert_name="GFP",
        ...
        mock_plot = MagicMock()
        with patch("src.genbank_utils.annotate", return_value=_empty_df()), \
             patch("src.genbank_utils.get_bokeh", return_value=mock_plot), \
             patch("src.genbank_utils.json_item", return_value={"doc": {}}):
    ...
    def test_annotate_called_once(self):
        ...
        mock_plot = MagicMock()
        with patch("src.genbank_utils.annotate", return_value=_empty_df()) as mock_annotate, \
             patch("src.genbank_utils.get_bokeh", return_value=mock_plot), \
             patch("src.genbank_utils.json_item", return_value={"doc": {}}):
